...
class VillageHouse(id: String) :
    ...
    override fun getSize(p_210446_: StructureManager, p_210447_: Rotation): Vec3i {
        val size = super.getSize(p_210446_, p_210447_)
        return size
    }
}
...
object StructureModule : ModModule {
    ...
    fun registerVillageHouses() {
        ...
        villages.forEach { (biome, pool) ->
            ...
            val newPool = StructureTemplatePool(pool.name, pool.fallback, pieces.object2IntEntrySet().map { Pair(it.key, it.intValue) })
            pools.registerOrOverride(OptionalInt.of(id), ResourceKey.create(pools.key(), pool.name), newPool, Lifecycle.stable())
        }
    }

    override fun generateLoot(builder: LootBuilder) {
        builder.add("chests/village/wine_cellar",
            LootTable.lootTable().withPool(LootPool.lootPool()
                .setRolls(ConstantValue.exactly(3F))
                .name("grapes")
                .`when`(ConfigLootCondition.forOption(Configs.SERVER.ENABLE_GRAPES))
                .add(LootItem.lootTableItem(GrapesModule.GRAPES)
        ...
        builder.add("chests/village/pub",
            LootTable.lootTable().withPool(LootPool.lootPool()
                ...
                .add(LootItem.lootTableItem(AlcoholModule.APPLE_WINE)
                    .`when`(ConfigLootCondition.forOption(Configs.SERVER.ENABLE_ALCOHOL))
                )
                .add(LootItem.lootTableItem(AlcoholModule.BEER).setWeight(3)
                    .`when`(ConfigLootCondition.forOption(Configs.SERVER.ENABLE_ALCOHOL))
                )
                .add(LootItem.lootTableItem(AlcoholModule.DARK_BEER).setWeight(3)
                    .`when`(ConfigLootCondition.forOption(Configs.SERVER.ENABLE_ALCOHOL))
                ).add(LootItem.lootTableItem(JuiceModule.APPLE_JUICE)
                    .`when`(InvertedLootItemCondition.invert(ConfigLootCondition.forOption(Configs.SERVER.ENABLE_ALCOHOL)))
                )
            ),
            LootContextParamSets.CHEST)
